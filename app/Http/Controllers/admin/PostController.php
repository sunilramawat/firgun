...
class PostController extends Controller{
     ...
     */
		...
		*/		if(strtolower($extension) == 'jpg' || strtolower($extension) == 'png' || strtolower($extension) == 'jpeg' ) {
 		...
 		if($validate->fails()){ 
			...
			$price_dec = $request->discount_price/$request->price*100;
			$offer = 100 - $price_dec;
			$request->offer = number_format((float)$offer, 2, '.', '');
   			$form_data 	= array(
				'title' => $request->title ?? null,
				'imgUrl' => $upload ? $upload :  $data['image_url'],
				'is_url' => $request->is_url ?? null,
				'u_id' => 1,
	...
	public function view(Request $request){
		...
		return view('admin.Post.view',compact('Partner'));
